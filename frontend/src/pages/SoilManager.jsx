...
import { Layers, Plus, Droplets, Trash2, Save, AlertCircle } from 'lucide-react';
...
export default function SoilManager() {
  ...
  const [formData, setFormData] = useState({
    ...
    tasa_max_infiltracion: '',    // NUEVO CAMPO REQUERIDO
    profundidad_radicular_max: '1.5' // NUEVO CAMPO REQUERIDO (Default 1.5m)
  });

  // PRESETS FAO MEJORADOS: Incluyen Infiltración estimada (mm/h)
  ...
  const fetchSoils = async () => {
    try {
      ...
      if (error.response?.status !== 404) {
        toast.error('Error cargando suelos');
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleTextureChange = (e) => {
    ...
    setFormData(prev => ({
      ...
      tasa_max_infiltracion: preset ? preset.inf : prev.tasa_max_infiltracion, // Autocompletar infiltración
    }));
  };
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      // CÁLCULO AUTOMÁTICO DE HUMEDAD DISPONIBLE
      // El backend lo pide, así que se lo calculamos aquí
      ...
      console.error(error);
      // Mostrar mensaje específico si el backend nos dice qué falló
      const errorData = error.response?.data;
      const msg = errorData 
        ? Object.entries(errorData).map(([k,v]) => `${k}: ${v}`).join(', ') 
        : 'Error al guardar el suelo.';
      toast.error(msg);
    }
  };
    ...
    <div className="max-w-6xl mx-auto">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-500">Configura las propiedades hidrodinámicas de tus terrenos.</p>
        </div>
        <button 
      ...
      {showForm && (
        <div className="bg-white p-6 rounded-xl shadow-lg border border-amber-100 mb-8 animate-fade-in-down">
          <h3 className="font-bold text-gray-800 mb-4 border-b pb-2">Registrar Nuevo Tipo de Suelo</h3>
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input 
                  type="text" required placeholder="Ej: Lote Sur"
              ...
              <div>
                ...
                <input 
                  ...
                  className="w-full p-2 border rounded focus:ring-2 focus:ring-amber-500 outline-none"
                  value={formData.profundidad_radicular_max}
                  onChange={e => setFormData({...formData, profundidad_radicular_max: e.target.value})}
                />
                <p className="text-xs text-gray-400 mt-1">Límite físico para raíces.</p>
              </div>
            </div>

            {/* COLUMNA 2: Hidráulica */}
            <div className="space-y-4">
              <div>
                ...
                <input 
                  ...
                  onChange={e => setFormData({...formData, capacidad_campo: e.target.value})}
                />
              </div>
              <div>
                ...
                <input 
                  type="number" step="0.1" required
                  className="w-full p-2 border rounded focus:ring-2 focus:ring-amber-500 outline-none"
                  value={formData.punto_marchitez}
                  onChange={e => setFormData({...formData, punto_marchitez: e.target.value})}
                />
              </div>
            </div>
            ...
            <div className="space-y-4 flex flex-col justify-between">
              <div>
                ...
                <input 
                  ...
                  onChange={e => setFormData({...formData, densidad_aparente: e.target.value})}
                />
              </div>
              <div>
                ...
                <input 
                  type="number" step="0.1" required
                  className="w-full p-2 border rounded focus:ring-2 focus:ring-amber-500 outline-none"
                  value={formData.tasa_max_infiltracion}
                  onChange={e => setFormData({...formData, tasa_max_infiltracion: e.target.value})}
                />
              </div>
          ...
          <div key={soil.id} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden group hover:shadow-md transition-all">
            ...
            <div className="p-5">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <p className="text-xs text-gray-500 uppercase font-bold">Agua Útil (Calculada)</p>
              ...
              <div className="grid grid-cols-2 gap-2 text-sm border-t pt-3 border-gray-100">
                <div className="flex justify-between">
                  <span className="text-gray-500">Infiltración:</span>
                  <span className="font-medium">{soil.tasa_max_infiltracion} mm/h</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">Profundidad:</span>
                  <span className="font-medium">{soil.profundidad_radicular_max} m</span>
                </div>
              </div>
            </div>
