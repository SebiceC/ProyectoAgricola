...
urlpatterns = [
    ...
    path(
        "api/register/", UserViewSet.as_view({"post": "register"}), name="api-register"
    ),
    path(
        "api/verify-otp/",
        UserViewSet.as_view({"post": "verify_otp"}),
        name="api-verify-otp",
    ),
    path(
        "api/refresh-token/",
    ...
    path(
        "api/session-status/",
        UserViewSet.as_view({"get": "session_status"}),
        name="api-session-status",
    ),
    path(
        "api/users/", UserViewSet.as_view({"get": "list_users"}), name="api-list-users"
    ),
    path(
        "api/users/<int:pk>/",
        UserViewSet.as_view({"get": "list_user_by_id"}),
