...
class UserRegisterSerializer(serializers.ModelSerializer):
    ...
    document_id = serializers.CharField(
        required=True,
        error_messages={"blank": "El documento de identidad no puede estar vacío."},
    )
    ...
    pais = serializers.CharField(
        required=True, error_messages={"blank": "El país no puede estar vacío."}
    )
    ...
    telefono = serializers.CharField(
        required=True, error_messages={"blank": "El teléfono no puede estar vacío."}
    )
    ...
    def validate_first_name(self, value):
        ...
        if "  " in value:
            raise serializers.ValidationError(
                "El nombre no puede contener espacios múltiples."
            )

        # Validar que no empiece ni termine con espacio
        if value.startswith(" ") or value.endswith(" "):
            raise serializers.ValidationError(
                "El nombre no puede empezar ni terminar con espacio."
            )
    ...
    def validate_last_name(self, value):
        ...
        if "  " in value:
            raise serializers.ValidationError(
                "El apellido no puede contener espacios múltiples."
            )

        # Validar que no empiece ni termine con espacio
        if value.startswith(" ") or value.endswith(" "):
            raise serializers.ValidationError(
                "El apellido no puede empezar ni terminar con espacio."
            )
...
class OTPVerificationSerializer(serializers.Serializer):
    ...
    def validate_otp(self, value):
        if not value.isdigit():
            raise serializers.ValidationError(
                "El código OTP debe contener solo números."
            )
        return value
...
class UserWithGroupSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = [
            "id",
            "username",
        ]  # Hacer el ID y el username de solo lectura
