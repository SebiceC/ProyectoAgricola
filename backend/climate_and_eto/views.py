...
class EvapotranspirationViewSet(viewsets.ViewSet):
    ...
    def get_evapotranspiration(self, request):
        ...
        if not lat or not lon:
            return Response(
                {"status": "error", "message": "Parámetros lat y lon son requeridos"},
        ...
        try:
            response = requests.get(
                "https://power.larc.nasa.gov/api/temporal/daily/point", params=params
            )
            ...
            evapotranspiration = (
                data.get("properties", {}).get("parameter", {}).get("EVLAND", {})
            )
    ...
    def _calculate_penman_monteith(self, tmax, tmin, rh_mean, wind_2m, rn, altitude):
        ...
        numerator = 0.408 * delta * rn + gamma * (900 / (tmean + 273)) * wind_2m * (
            es - ea
        )
