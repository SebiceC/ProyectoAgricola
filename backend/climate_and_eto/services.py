...
class NasaPowerService:
    ...
    def computar_eto(self, t_min, t_max, radiation, humidity, wind_speed, elevation):
        ...
        rn -= 4.903e-9 * (((t_max + 273.16)**4 + (t_min + 273.16)**4)/2) * (0.34 - 0.14*math.sqrt(ea)) * (1.35*(radiation/20) - 0.35)
        
        # ETO final
        numerator = 0.408 * delta * rn + gamma * (900/(t_mean + 273)) * wind_speed * (es - ea)
